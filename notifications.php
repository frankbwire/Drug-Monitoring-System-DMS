...
<body class="text-monospace">
    ...
    <div class="container border rounded shadow-sm">
       <h4 class="text-center border rounded shadow" style="font-weight: bold; margin-top:10px;">Messages</h4>
          <hr>
        <p style="font-size: 14px;">The table show all messages received.</p>
        
        <div class="table-responsive table-bordered text-monospace" style="height: 300px;font-size: 13px;">
             <?php
            //
            $sql4="SELECT * FROM `notifications` order by `date_received` desc;";
            $query4=mysqli_query($connect,$sql4) or die  (mysqli_error($connect));
            ?>
            <table class="table table-striped table-bordered table-sm">
                <thead style="background-color: rgba(149,247,147,0.9);">
                    <tr>
                        <th>Date Received</th>
                        <th>Message</th>
                    </tr>
                ...
                <tbody>
                    <tr>
                      <td><?php echo $result4["date_received"];  ?></td>
                       <td><?php echo $result4["message"];  ?></td>
                        
<td class="text-center"><a class="btn btn-danger btn-sm text-white" role="button" href="delete.php?not=<?php echo $result4["notification_id"]; ?>" style="padding-top:0px;padding-bottom:0px; font-size:12px;">Delete</a></td>
                    </tr>
                </tbody>
                <?php
                };
